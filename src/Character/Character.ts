...
export default class Character {
    ...
    get_pos_toward(target: Vector) : Vector {
        var xDiff = Math.abs(this.#pos.x - target.x);
        var yDiff = Math.abs(this.#pos.y - target.y);
        if (xDiff > yDiff) {
    ...
    move_random(otherPos: Vector): void {
        let dist = this.#distance(this.#pos, otherPos);
        if (dist > 8) {
            let dir = this.get_pos_toward(otherPos);
            this.#pos.x += dir.x;
            this.#pos.y += dir.y;
        }
        else if (dist < 3) {
