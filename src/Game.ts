...
export class Game_Darkness {
    ...
    on_input() {
        ...
        try
        {
            ...
            {
                if (input === "USE" && clean(args[0]) === "HANDS") { // Using your hands on smth is the same as touching it
                    input = "TOUCH";
                    args = [args[1]];
                }

                let choices = this.#actionManager.getActions(this.#screen, this.#characterManager);
